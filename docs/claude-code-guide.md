...
**Node.js란?**: JavaScript를 실행하는 프로그램입니다. Claude Code가 Node.js를 사용하여 작동합니다.
...
2. "LTS" 버전 다운로드 (Long Term Support)
3. 다운로드한 `.pkg` 파일 실행
4. 설치 마법사 따라하기
5. 터미널 재시작 후 `node --version` 확인

**방법 2: Homebrew로 설치** (개발자용)
```bash
# Homebrew 설치 (없는 경우)
...
**기본 터미널 사용**:
- macOS에 기본 설치된 "터미널" 앱 사용 가능
- 위치: `/Applications/Utilities/Terminal.app`
- Spotlight 검색: `⌘ + Space` → "터미널" 입력

**대안 (선택사항)**:
- iTerm2: https://iterm2.com/ (개발자에게 인기)
- Warp: https://www.warp.dev/ (현대적인 터미널)

#### 4. 인터넷 연결

- Claude Code는 Anthropic 서버와 통신이 필요합니다
- 안정적인 인터넷 연결 필수
...
#### 방법 1: setup.sh 스크립트 사용 (권장)

**가장 쉬운 방법**: AI-driven-work 프로젝트의 자동 설정 스크립트를 사용하면 Claude Code 설치와 모든 설정이 자동으로 완료됩니다.

```bash
# 1. 터미널 열기
# Spotlight (⌘ + Space) → "터미널" 입력 → Enter

# 2. AI-driven-work 프로젝트로 이동
...
**스크립트가 자동으로 처리하는 것**:
- ✅ Node.js 버전 확인
- ✅ Claude Code 설치 (없는 경우)
- ✅ MCP Server 설정 (Jira/Confluence 연동)
- ✅ Slash Commands 설정
- ✅ 연결 테스트
...
```
==========================================
🚀 POPUP STUDIO AI-Driven Work
==========================================

이 스크립트는 Claude Code와 Atlassian MCP Server를 설정합니다.
...
✅ Node.js v20.10.0 확인됨

==========================================
🤖 2단계: Claude Code 설치
==========================================

⚠️  Claude Code가 설치되어 있지 않습니다.

Claude Code를 설치하시겠습니까? (y/n): y
...
**자동 스크립트를 사용할 수 없는 경우** 수동으로 설치할 수 있습니다:
...
Claude Code가 제대로 설치되었는지 확인하세요:

```bash
# 1. 설치 확인
which claude
# 출력: /usr/local/bin/claude (또는 유사한 경로)
...
**원인**: Claude Code가 설치되지 않았거나 PATH에 등록되지 않음
...
# 2. Node.js 업데이트
# 방법 A: 공식 웹사이트에서 최신 버전 다운로드
# https://nodejs.org/

# 방법 B: Homebrew 사용 (이미 Homebrew로 설치한 경우)
brew upgrade node
```
